...
list_projects () {
 ...
 #   #check if gitlab-ci.yml exists
    local project_name=$(curl --silent --header "Private-Token:$GITLAB_ACCESS_TOKEN" "https://gitlab.devops.telekom.de/api/v4/groups/$group_id/projects" | jq -r '.[].name')
    local project_id=$(curl --silent --header "Private-Token:$GITLAB_ACCESS_TOKEN" "https://gitlab.devops.telekom.de/api/v4/groups/$group_id/projects" | jq -r '.[].id')
...
#    local projects_ids=$(curl --silent --header "Private-Token:$GITLAB_ACCESS_TOKEN" "https://gitlab.devops.telekom.de/api/v4/groups/$group_id/projects" | jq -r '.[].id')
#    # Print projects in the current subgroup
#    echo "Projects in $group_name:"
#    echo "Project name:$projects"
#    echo "Project id:$project_ids"
#
